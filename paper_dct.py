...
class PaperDCT:
    ...
    def __init__(self, quant_level=100, num_used_channels=1, n_bits_per_block=1, end_string='$t3g0'):
        '''
            ...
            quant_level: dung de tao quantization table
        ...
        self.T = self._get_DCT_matrix()
    def __repr__(self) -> str:
        return f'DCT(num_used_channels={self.num_used_channels}, quant_level={self.quant_level}, n_bits_per_block={self.n_bits_per_block})'
    ...
    def _encode_2d_matrix(self,matrix, bits):
        ...
        index = 0
        for i in range(len(quantizedBlocks)):
            self._replace_DC_bit(quantizedBlocks[i], bits[index])
            index+=1
            if index == len(bits):
                break
    ...
    def encode(self, img_path, message, dest_path, grayscale=False):
        ...
        pil_im = Image.fromarray(np.round(np.clip(newImg,0,255)).astype(np.uint8))
        pil_im.save(dest_path, mode=mode)
    ...
    def decode(self,img):
        ...
        if mode == 'RGB':
            ...
            for i in range(self.num_used_channels):
                ...
                for block in quantizedBlocks:
                    b += str(int(block[0][0]%2))
                    if len(b)==8:
                        msg += chr(int(b,2))
                        b =''
...
for path in paths:
    d = 'encoded_images/'+ path.split('/')[-1].split('.')[0] + '_' + 'paperdct.png'
    encoded = dct.encode(path,message,d)
    newMessage = dct.decode(encoded)
